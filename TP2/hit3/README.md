# Hit #3 y Aplicación Base — Logs JSON Estructurados

Este directorio contiene la aplicación de Scraping refactorizada para emitir logs en formato JSON estructurado (Hit #3), y la implementación del Stack de Observabilidad.
...
> **IMPORTANTE HIT #3**: Hemos agregado `python-json-logger` a las dependencias. ¡Asegúrate de compilar la imagen nuevamente!
...
#### 4. Disparar tráfico y validar labels en Grafana (Hit #3)
...
```
Si revisas la columna *Detected fields*, deberás ver campos como `producto`, `level`, `logger`, `browser`, etc. extraídos directamente del JSON.

#### 📸 Consigna obligatoria (Hit #3): Antes y Después
Debes capturar y guardar dos screenshots en la carpeta `observability/screenshots/` para documentar la diferencia.

1. **Antes (Hit #2 - Texto Plano)**: Captura un log antiguo y guárdalo como `hit3-antes.png`.
   *(Muestra la línea logfmt pura en un solo string)*.
2. **Después (Hit #3 - JSON Estructurado)**: Captura un log nuevo usando `| json` y guárdalo como `hit3-despues.png`.
   *(Asegúrate de expandir el log en Grafana para mostrar todos los campos extraídos)*.

![Hit 2 Antes](observability/screenshots/hit3-antes.png)
![Hit 3 Después](observability/screenshots/hit3-despues.png)
