            ...
            for intento in range(1, max_attempts + 1):
                try:
                    ...
                    if intento == max_attempts:
                        logger.error(
                            "Fallo total tras agotar reintentos",
                            extra={"event": "retry_exhausted", "function": getattr(func, '__name__', str(func)), "max_attempts": max_attempts, "error": type(e).__name__},
                            exc_info=True
                        )
                        raise e
                    ...
                    logger.warning(
                        "Reintento en progreso",
                        extra={"event": "retry_attempt", "function": function_name, "attempt": intento, "max_attempts": max_attempts, "delay": delay, "error": type(e).__name__}
                    )
                    time.sleep(delay)
