...
def save_json(results: list[dict], product: str) -> None:
    ...
    logger.info("JSON escrito exitosamente", extra={"event": "json_written", "path": str(out_path)})
...
def main() -> None:
    ...
    logger.info(
        "Scraper iniciado",
        extra={"event": "scraper_start", "productos": len(products), "browser": browser, "headless": headless, "max_pages": max_pages, "delay": DELAY_BETWEEN_PRODUCTS}
    )

    # Ejecutar migrations de Postgres (solo si está habilitado)
    if postgres_enabled():
        logger.info("Postgres habilitado — ejecutando migrations", extra={"event": "postgres_migrations_start"})
        run_migrations()
    ...
    try:
        for idx, product in enumerate(products):
            logger.info("Iniciando producto", extra={"event": "product_start", "producto": product})
            try:
                ...
                if results:
                    ...
                    logger.info("Guardado JSON exitoso", extra={"event": "json_save_success", "producto": product, "items": len(results)})
                else:
                    logger.warning("Sin resultados, no se escribe JSON", extra={"event": "product_empty", "producto": product})
                ...
                logger.info("Producto scrapeado exitosamente", extra={"event": "product_success", "producto": product})

            except Exception as e:
                logger.error("Error al scrapear producto", extra={"event": "product_error", "producto": product, "error": type(e).__name__}, exc_info=True)

            # Pausa entre productos para evitar throttling (excepto el último)
            if idx < len(products) - 1:
                logger.info("Esperando antes del siguiente producto", extra={"event": "throttling_wait", "delay": DELAY_BETWEEN_PRODUCTS})
                time.sleep(DELAY_BETWEEN_PRODUCTS)

    except Exception as e:
        logger.critical("Error fatal en el scraper", extra={"event": "scraper_fatal_error", "error": type(e).__name__}, exc_info=True)
    finally:
        try:
            driver.quit()
            logger.info("Driver cerrado exitosamente", extra={"event": "driver_quit_success"})
        except Exception as e:
            logger.error("Error al cerrar driver", extra={"event": "driver_quit_error", "error": type(e).__name__}, exc_info=True)
