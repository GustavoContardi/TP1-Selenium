...
def compute_stats(results: list[dict], product: str) -> dict:
    ...
    if not precios:
        logger.warning(
            "Sin precios válidos para estadísticas",
            extra={"event": "stats_no_prices", "producto": product}
        )
        return {
            "producto": product,
...
def save_stats(all_stats: list[dict], output_dir: str = "output") -> None:
    ...
    logger.info("Estadísticas guardadas exitosamente", extra={"event": "stats_save_success", "path": path})
