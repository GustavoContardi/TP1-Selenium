...
def get_connection():
    ...
    try:
        ...
        logger.error("Error al conectar con la base de datos", extra={"event": "db_connection_error", "error": type(e).__name__}, exc_info=True)
        raise
        ...
        with conn:
            ...
            for path in sorted(glob.glob(f"{migrations_dir}/*.sql")):
                ...
                with conn.cursor() as cur:
                    ...
                    if cur.fetchone():
                        logger.debug(
                            "Migration ya aplicada, saltando",
                            extra={"event": "migration_skipped", "migration_file": filename}
                        )
                        continue
                    ...
                    logger.info("Migración aplicada", extra={"event": "migration_applied", "migration_file": filename})

        logger.info("Migraciones finalizadas exitosamente", extra={"event": "migrations_completed"})
    except Exception as e:
        logger.error("Fallo durante migraciones", extra={"event": "migrations_error", "error": type(e).__name__}, exc_info=True)
        raise
    finally:
        conn.close()
...
def save_results(results: list[dict], product: str, browser: str) -> int:
    ...
    if not results:
        logger.warning(
            "Sin resultados para persistir",
            extra={"event": "db_insert_empty", "producto": product}
        )
        return 0
        ...
        with conn:
            with conn.cursor() as cur:
                for r in results:
                    ...
                    inserted += 1
        logger.info(
            "Resultados guardados en la BD",
            extra={"event": "db_insert_success", "producto": product, "filas": inserted}
        )
    except Exception as e:
        logger.error(
            "Error al persistir resultados",
            extra={"event": "db_insert_error", "producto": product, "error": type(e).__name__},
            exc_info=True,
        )
    finally:
        conn.close()
