...
def extract_titulo(element, context: dict):
    try:
        ...
        logger.warning(
            "Error al extraer título",
            extra={"event": "field_extraction_error", "field": "titulo", "producto": context.get("producto"), "browser": context.get("browser"), "error": type(e).__name__}
        )
        return None
...
def extract_precio(element, context: dict):
    try:
        ...
        logger.warning(
            "Error al extraer precio",
            extra={"event": "field_extraction_error", "field": "precio", "producto": context.get("producto"), "browser": context.get("browser"), "error": type(e).__name__}
        )
        return None
...
def extract_link(element, context: dict):
    try:
        ...
        logger.warning(
            "Error al extraer link",
            extra={"event": "field_extraction_error", "field": "link", "producto": context.get("producto"), "browser": context.get("browser"), "error": type(e).__name__}
        )
        return None
...
def extract_tienda_oficial(element, context: dict):
    try:
        ...
        logger.warning(
            "Filtro tienda_oficial no disponible",
            extra={"event": "field_missing", "field": "tienda_oficial", "producto": context.get("producto"), "browser": context.get("browser"), "error": type(e).__name__}
        )
        return None
...
def extract_envio_gratis(element, context: dict):
    try:
        ...
        logger.warning(
            "Filtro envio_gratis no disponible",
            extra={"event": "field_missing", "field": "envio_gratis", "producto": context.get("producto"), "browser": context.get("browser"), "error": type(e).__name__}
        )
        return False
...
def extract_cuotas(element, context: dict):
    try:
        ...
        logger.warning(
            "Filtro cuotas_sin_interes no disponible",
            extra={"event": "field_missing", "field": "cuotas_sin_interes", "producto": context.get("producto"), "browser": context.get("browser"), "error": type(e).__name__}
        )
        return None
